class colorPicker {
  ...
  public void update(int cursorx, int cursory){
     if(hitTest(cursorx, cursory)){
        image(cpImage, mx, my);
        mc = get( cursorx, cursory ); 
        println("cursorX: " + cursorx + " cursorY: " + cursory + " COLOUR: " + mc);
     }
  }
  
  
  public boolean hitTest(int cursorx,int cursory){
  ...
  public void render (){
    image(cpImage, mx, my);
    if ( mousePressed && mouseX >= mx && mouseX < mx + mw && mouseY >= my && mouseY < my + mh ) {
      mc = get( mouseX, mouseY );
      println("chosen color: " + mc);
    }
    buffer.fill(mc); 
    // selected colour rectangle
    buffer.rect(mx+300, my-50, 40, mh);
  }
